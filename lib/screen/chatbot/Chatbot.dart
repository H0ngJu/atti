...
class Chatbot {
  ...
  Future<String> getResponse(var prompt, String img, DocumentReference docRef) async {
    ...
    final model = GenerativeModel(
      ...
      systemInstruction: Content.text(
          '너는 치매 어르신과 대화를 나누는 보이스봇 아띠야. 한국어로만 말해야해. 어르신이 화면 속의 사진에 담긴 추억을 회상하는 것을 돕는 것이 목적이야. 너의 역할은 질문에 대한 정보를 제공하는 것이 아니야. 어르신께 사진에 대해 물어봐줘. 사진의 정보를 바탕으로 공감하고 긍정적인 정서를 이끌어내도록 어르신께 대화를 유도해줘. 그리고 너무 길게 말하지 말아줘.'
      ),
    );
  ...
  Future<void> emotionAnalysis(String messages) async {
    ...
    final model = GenerativeModel(
      ...
      requestOptions: RequestOptions(apiVersion: 'v1beta'),
      systemInstruction: Content.text(
          '이 대화에서 사용자가 느낀 감정들을 분석해줘. 기쁨, 그리움, 슬픔, 즐거움 이런 식의 짧은 단어로 2~3개 정도 말해줘. 감정 키워드만 말해주면 돼.'
      ),
    );
